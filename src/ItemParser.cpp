        ...
        for(rapidxml::xml_node<>* itm = doc.first_node("World")->first_node("item"); itm; itm = itm->next_sibling("item"))
        {
            ...
            if(type)
            {
                ...
                if(type->value() == "weapon")
                {
                    ...
                    ItemsBank::GetInstance().AddItem(ID, std::unique_ptr<Item>(new Weapon(name, description, value, atk, def)));
                }
                else if(type->value() == "armour")
                {
                    int slot = std::stoi(itm->first_node("slot")->value());
                    ItemsBank::GetInstance().AddItem(ID, std::unique_ptr<Item>(new Armour(name, description, value, def, slot)));
                }
            }
            //Otherwise it's plain boring item.
            else
            {
                ItemsBank::GetInstance().AddItem(ID, std::unique_ptr<Item>(new Item(name, description, value)));
            }
